...
from utils.attention_visualization import save_attention_over_images
from utils.create_geojson import get_country_predictions_geojson

IMAGES_DIR = '/Volumes/Virginia2/Hotel50k/'  #directory delle immagini
HOTEL_INFO_DIR = '/Volumes/Virginia2/floyd_data/data/'   #directory dei file csv relativi alle immagini

ImageFile.LOAD_TRUNCATED_IMAGES = True #a cosa serve?
...
class HotelDataGenerator(DataGenerator):
    ...
    def __init__(self, bs):
        ...
        self.images_per_city_train = defaultdict(list) #defaultdict means that if a key is not found in the dictionary, then instead of a KeyError being thrown, a new entry is created.
        for image, hotel in self.image_hotel_train_dict.items():
    ...
    def generate_batch(self, train=True):
        while True:
            ...
            city_batch = np.random.choice(a=cities, size=self.bs, replace=True) #n.b. la generazine random può pescare due volte lo stesso elemento
            city_frequency = Counter(city_batch) #counter mi restituisce un dizionario con chiave la città e valore la frequenza
            for city, freq in city_frequency.items():
                ids = np.random.choice(a=images_per_city[int(city)], size=freq, replace=True)
                for i, id in enumerate(ids):  #i ti da l' indicizzazione, id il contenuto
...
class LearnToPayAttention:
    ...
    def test_model(self, image_path):
        ...
        ordered1 = p1.argsort()[::-1][:10] #cosi mi da in ordine decrescente le top 10
