...
class Swin_b_TransformerFineTuning(
    ...
    def _load_model(self):
        ...
        self.pretrained = swin_transformer.SwinTransformer(**model_config)
        self.pretrained_path = 'pretrained_models/swin_base_patch4_window7_224_22k.pth'
        self.pretrained.load_state_dict(torch.load(self.pretrained_path)[
                                            'model'])
        config = dict(
            embed_dim=128,  #prima era 96
            depths=[2, 2, 6, 2],
            num_heads=[3, 6, 12, 24],
            window_size=7,
            ape=False,
        ...
        checkpoint = torch.load('/home/rozenberg/indoors_geolocation_pycharm/pretrained_models/swin_base_patch4_window7_224_22k.pth')
        #print(checkpoint['model'].keys())
        #checkpoint_backbone = {k.replace('backbone.', ''): v for k, v in checkpoint['model'].items() if
        #                       k.startswith('backbone')}
        #model.load_state_dict(checkpoint['model'].items())
