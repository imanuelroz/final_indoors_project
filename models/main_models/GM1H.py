...
class LearnToPayAttention:
    ...
    def create_model(self):
        ...
        country_softmax = tf.keras.layers.Dense(self.COUNTRY_CLASSES, activation='softmax')(g)
        city_softmax = tf.keras.layers.Dense(self.CITY_CLASSES, activation='softmax')(g)
        subregion_softmax = tf.keras.layers.Dense(self.SUBREGION_CLASSES, activation='softmax')(g)
        chain_softmax = tf.keras.layers.Dense(self.CHAIN_CLASSES, activation='softmax')(g)

        self.model = tf.keras.Model(inputs=self.vgg.input,
                                    outputs=[country_softmax, city_softmax, subregion_softmax, chain_softmax]) #

    def compile_model(self, lr=1e-6, metrics=['accuracy']):
...
if __name__ == "__main__":
    ...
    geolocation_model.model.set_weights(pickle.load(open("weights/gm1h.pkl", "rb")))
