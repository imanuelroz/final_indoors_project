...
class LearnToPayAttention:
    ...
    def create_model(self):
        ...
        # OUTPUT LAYER
        g_layers = tf.keras.layers.Concatenate()([g, g1, g2, g3])
    ...
    def compile_model(self, lr=1e-7, metrics=['accuracy']):
    ...
    def test_model(self, image_path):
        ...
        probs1 = [p1[i] for i in ordered1]

        p2 = predictions[1][0]
        ordered2 = p2.argsort()[::-1][:10]
        probs2 = [p2[i] for i in ordered2]

        # SUBREGION prediction
        p3 = predictions[2][0]
        ordered3 = p3.argsort()[::-1][:5]
        probs3 = [p3[i] for i in ordered3]

        # CHAIN prediction
        p4 = predictions[2][0]
        ordered4 = p4.argsort()[::-1][:5]
        probs4 = [p4[i] for i in ordered4]
        ...
        return [probs1, probs2, probs3, probs4, ordered1, ordered2, ordered3, ordered4]
    ...
    def get_attention_maps(self, image_path, self_attention_paths):
        ...
        avg_attention = np.mean([cv2.resize(attention_map_0, dim, interpolation=cv2.INTER_NEAREST), cv2.resize(attention_map_1, dim, interpolation=cv2.INTER_NEAREST), cv2.resize(attention_map_2, dim, interpolation=cv2.INTER_NEAREST)], axis=0)
...
if __name__ == "__main__":
    ...
    geolocation_model.model.set_weights(pickle.load(open("weights/gm3h.pkl", "rb")))
